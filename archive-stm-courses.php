<?php
/**
 * Custom Post Type Archive Template for stm-courses
 * 
...
// Filter by price (free) if selected
        ...
        <div class="courses-catalog-layout" style="display: grid; grid-template-columns: 280px 1fr; gap: 40px; align-items: start;">
            ...
            <aside class="catalog-sidebar" style="background: #ffffff; padding: 25px; border-radius: 16px; border: 1px solid #e2e8f0; box-shadow: 0 4px 15px rgba(15, 23, 42, 0.02); position: sticky; top: 120px;">
                ...
                <ul style="list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 12px;">
                    ...
                    $all_active = empty($category_filter) && empty($price_filter) ? 'color: #007bff; font-weight: 700;' : 'color: #475569;';
                    echo '<li><a href="' . esc_url( remove_query_arg( array( 'category', 'price' ) ) ) . '" style="' . $all_active . ' text-decoration: none; font-size: 15px; font-family: \'Kantumruy Pro\', sans-serif !important; display: block; padding: 6px 0; transition: color 0.2s ease;">📚 វគ្គសិក្សាទាំងអស់</a></li>';

                    $free_active = ($price_filter === 'free') ? 'color: #10b981; font-weight: 700;' : 'color: #475569;';
                    $free_link = add_query_arg( 'price', 'free', remove_query_arg( 'category' ) );
                    echo '<li><a href="' . esc_url( $free_link ) . '" style="' . $free_active . ' text-decoration: none; font-size: 15px; font-family: \'Kantumruy Pro\', sans-serif !important; display: block; padding: 6px 0; transition: color 0.2s ease;">🎁 វគ្គសិក្សាឥតគិតថ្លៃ</a></li>';
                    ...
                    if ( ! is_wp_error( $categories ) && ! empty( $categories ) ) {
                        foreach ( $categories as $cat ) {
                            ...
                            if ( ! empty( $price_filter ) ) {
                                $link = add_query_arg( 'price', $price_filter, $link );
                            }
            ...
            <div class="catalog-main">
                        ...
                        while ( $courses_query->have_posts() ) : $courses_query->the_post();
                            ...
                            <article class="course-card">
                                ...
                                <div class="course-content">
                                    ...
                                    $enroll_url = $use_custom_enroll ? home_url( '/enroll/?course_id=' . get_the_ID() ) : get_permalink();
                                    ?>
                                    <a href="<?php echo esc_url( $enroll_url ); ?>" class="btn-enroll">ចូលរៀនឥឡូវនេះ</a>
                                </div>
                            </article>
                            <?php
                        endwhile;
                ...
                if ( $total_pages > 1 ) {
                    // Build the base URL for pretty /courses/page/%#%/ pagination
                    $courses_base_url = home_url( '/courses/' );
